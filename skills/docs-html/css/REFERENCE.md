...
| `charts.css` | **shared, engine-agnostic**: the `.chart-figure` card (the validated `bg-soft` surface), the `.chart-canvas` an engine draws into, the `.chart-tools` toolbar, and the `pre.chart` readable-spec fallback — one definition for every engine, selected by the shared `chart` marker class. The categorical palette is data, not CSS: it lives in `js/modules/charts.js` (with the sequential ramp and the semantic direction tones) and is verified by `python builder.py dataviz`. Also `.chart-note`, the one-line reading under a chart |
